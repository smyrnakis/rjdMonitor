...
// char talosUrlFire[] = TALOS_URL_FIRE;
// char talosUrlMovement[] = TALOS_URL_MOVEMENT;
...
const char* thinkSpeakAPIurl = "http://api.thingspeak.com"; // "184.106.153.149" or api.thingspeak.com
const char* autoRemoteURL = "http://autoremotejoaomgcd.appspot.com";
...
// Send to TALOS - movement
void sendToTALOS_movement() {
  clientSecure.setInsecure();
  clientSecure.connect(TALOS_URL_MOVEMENT, 443);

  clientTalos.begin(clientSecure, TALOS_URL_MOVEMENT);

  int httpCode = clientTalos.GET();

  String payload = clientTalos.getString();

  Serial.println(httpCode);
  Serial.println(payload);
}

void sendToTALOS_fire() {
  clientSecure.setInsecure();
  clientSecure.connect(TALOS_URL_FIRE, 443);

  clientTalos.begin(clientSecure, TALOS_URL_FIRE);
...
void movementReport_XmasLEDs() {
  clientXmasLEDs.setTimeout(8);
  clientXmasLEDs.begin(client, "http://192.168.1.31/movement");
...
String HTMLpresentData(){
  ...
  ptr += (String)millis();
  ptr += "</p>";
  ptr += "<p><b>Up time (millis):</b> ";
  ptr += millisToTime(true);
  ptr += "</p>";
  ptr += "<p></p>";
